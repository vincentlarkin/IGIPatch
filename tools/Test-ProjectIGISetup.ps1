...
$iniPath = Join-Path (Split-Path $GamePcDir -Parent) "IGIPatch.ini"
if (!(Test-Path -LiteralPath $iniPath -PathType Leaf)) {
    $iniPath = Join-Path $GamePcDir "IGIPatch.ini"
}
if (Test-Path -LiteralPath $iniPath -PathType Leaf) {
    Add-Result "IGIPatch.ini" "OK" "Found $iniPath"
} else {
    Add-Result "IGIPatch.ini" "WARN" "No IGIPatch.ini found near the game. Defaults will be used."
}
